...
type AccordionSliderProps = {
	...
	direction?: string;
	/** The button that triggers the restoration of the initial state, represented by class, such as `.poemui-accordion-img__close` */
...
export default class AccordionSlider extends Component<AccordionSliderProps, AccordionSliderState> {
	...
	handleClickItem(e) {
		...
		if ( window.innerWidth > 768 ) {
			...
			if ( e.target.className == 'poemui-accordion-slider__content__info' || e.target.className == 'poemui-accordion-slider__content' ) {
		...
		return (
		  <>
				...
				<div 
				    ref={this.rootRef}
					id={id || this.uniqueID}
					className="poemui-accordion-slider">
					<ul>
