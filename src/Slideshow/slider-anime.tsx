...
export function sliderAnime( curElement: any, config: sliderAnimeConfig ) {
    ...
    config = __.setDefaultOptions({
        ...
        "paginationID"      : ".poemui-slideshow__pagination",
        "arrowsID"          : ".poemui-slideshow__arrows",
        "draggable"         : false,
    ...
    function sliderInit(resize) {
        ...
        const $items = $sliderWrapper.find('.poemui-slideshow__item'),
              $first = $items.first();
    ...
    */
    function sliderAutoPlay( playTimes, timing, loop, slider, countTotalID, countCurID, paginationID, arrowsID ) {	
        ...
        const items = slider.find( '.poemui-slideshow__item' ),
              total = items.len();
    ...
    */
    function addItemsToStage( slider, nativeItemW, nativeItemH, paginationID, arrowsID, loop, draggable, draggableCursor, countTotalID, countCurID ) {
        ...
        const $this                    = slider,
                $items                   = $this.find( '.poemui-slideshow__item' ),
                $first                   = $items.first(),
        ...
        if ( __( arrowsID ).len() == 0 ) {
            __( 'body' ).prepend( '<div style="display:none;" class="poemui-slideshow__arrows '+arrowsID.replace('#','').replace('.','')+'"><a href="#" class="poemui-slideshow__arrows--prev"></a><a href="#" class="poemui-slideshow__arrows--next"></a></div>' );
        }
        ...
        //Next/Prev buttons
        //-------------------------------------		
        const _prev = __( arrowsID ).find( '.poemui-slideshow__arrows--prev' ),
              _next = __( arrowsID ).find( '.poemui-slideshow__arrows--next' );
        ...
        //-------------------------------------	
        const $dragTrigger = $this.find( '.poemui-slideshow__inner' );
        let mouseX, mouseY;
        let isMoving = false;
    ...
    */
    function sliderUpdates( elementIndex, slider, dir, countTotalID, countCurID, paginationID, arrowsID, loop ) {
        ...
        if ( loop ) {
            ...
            __( arrowsID ).find( 'a' ).removeClass( 'is-disabled' );
            if ( elementIndex == total - 1 ) __( arrowsID ).find( '.poemui-slideshow__arrows--next' ).addClass( 'is-disabled' );
            if ( elementIndex == 0 ) __( arrowsID ).find( '.poemui-slideshow__arrows--prev' ).addClass( 'is-disabled' );
        }
        ...
        if ( __.isTouchCapable() ) {
            ...
            if ( !loop ) {
                //first item
                if ( elementIndex == 0 ) {
                    __( arrowsID ).find( '.poemui-slideshow__arrows--prev' ).addClass( 'is-disabled' );
                }

                //last item
                if ( elementIndex == total - 1 ) {
                    __( arrowsID ).find( '.poemui-slideshow__arrows--next' ).addClass( 'is-disabled' );
                }	
            }
        ...
        slider.find( '.poemui-slideshow__item.is-active' ).removeClass( 'is-active' ).addClass( 'leave ' + dirIndicatorClassName );
    ...
    function itemDefaultInit( slider, currentLlement ) {
        ...
        if ( currentLlement.find( 'video' ).len() > 0 ) {
            ...
            currentLlement.find( 'video' ).css({
                'width': currentLlement.closest( '.poemui-slideshow__outline' ).width() + 'px'
            });   
            
            getVideoDimensions(_src).then(function (res: any): void {
                slider.css( 'height', res.height*(currentLlement.closest( '.poemui-slideshow__outline' ).width()/res.width) + 'px' );
            });
            ...
            if ( imgURL !== null ) {
                ...
                img.onload = function() {
                    ...
                    slider.css( 'height', currentLlement.closest( '.poemui-slideshow__outline' ).width()*(img.height/img.width) + 'px' );		
