...
export default class ShowMoreLess extends Component<ShowMoreLessProps, ShowMoreLessState> {
    ...
    createTrigger(tagName, classes, showContent, hideContent) {
        switch (tagName) {
            case 'a':
                return <a role="button" href="#" className={"poemui-more-btn__link " + classes} onClick={this.handleToggle}>{!this.state.displayEnabled ? <span>{showContent}</span> : <span>{hideContent}</span>}</a>;

            case 'button':
                return <button type="button" className={"poemui-more-btn__link " + classes} onClick={this.handleToggle}>{!this.state.displayEnabled ? <span>{showContent}</span> : <span>{hideContent}</span>}</button>;

            case 'div':
                return <div role="button" className={"poemui-more-btn__link " + classes} onClick={this.handleToggle}>{!this.state.displayEnabled ? <span>{showContent}</span> : <span>{hideContent}</span>}</div>;

            case 'span':
                return <span role="button" className={"poemui-more-btn__link " + classes} onClick={this.handleToggle}>{!this.state.displayEnabled ? <span>{showContent}</span> : <span>{hideContent}</span>}</span>;
        ...
        return (
            <>
                ...
                <div
                    ...
                    className={this.state.displayEnabled ? 'poemui-more-btn is-active' : 'poemui-more-btn'}
                    style={{
                ...
                <div className="poemui-more-btn__trigger">
