...
export default class Group extends Component<GroupProps, GroupState> {
    render() {
        ...
        return (
            <>
                {data.map((item, index) => {
                    return <div className={item.current ? 'poemui-cascading-dropdown-list__opt is-active' : 'poemui-cascading-dropdown-list__opt'} key={index} onClick={(e) => selectEv!(item, index)}>{item.name}</div>
                })}
            </>
        )
    }
