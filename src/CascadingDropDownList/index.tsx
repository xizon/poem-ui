...
export default class CascadingDropDownList extends Component<CascadingDropDownListProps, CascadingDropDownListState> {
     ...
     handleClickOutside(event) {
		if ( 
			event.target.className != '' && ( 
				event.target.className.indexOf( 'poemui-cascading-dropdown-list__trigger' ) < 0 && 
				event.target.className.indexOf( 'poemui-cascading-dropdown-list__items' ) < 0 && 
                event.target.className.indexOf( 'poemui-cascading-dropdown-list__opt' ) < 0
			)
		   ) {
    ...
    createTrigger(tagName, classes, content) {
        switch (tagName) {
            case 'a':
                return <a role="button" href="#" className={"poemui-cascading-dropdown-list__trigger " + classes} onClick={this.handleDisplayOptions}>{content}</a>;

            case 'button':
                return <button type="button" className={"poemui-cascading-dropdown-list__trigger " + classes} onClick={this.handleDisplayOptions}>{content}</button>;

            case 'div':
                return <div role="button" className={"poemui-cascading-dropdown-list__trigger " + classes} onClick={this.handleDisplayOptions}>{content}</div>;

            case 'span':
                return <span role="button" className={"poemui-cascading-dropdown-list__trigger " + classes} onClick={this.handleDisplayOptions}>{content}</span>;
    ...
    render() {
        ...
        return (
            <>
                ...
                <div
                    id={id || this.uniqueID}
                    className="poemui-cascading-dropdown-list" style={{zIndex:(depth ? depth : 1)}}>
                    ...
                    <em className="poemui-cascading-dropdown-list__result">{displayInfo}</em>

                    {isShow ? (
                        <div className="poemui-cascading-dropdown-list__items">
                            <ul>
                                {data.map((item, level) => {
                                    return (
                                        <li key={level}>
