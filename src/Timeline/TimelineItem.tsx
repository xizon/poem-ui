	...
	render() {
		...
		return (
			<>
				...
				<div className={"poemui-timeline__item" + activedClassName}>
					<div className="poemui-timeline__item__img poemui-border--circle poemui-border--circle-only-img" onClick={clickEv}>
						<img src={url} alt="" />
						{nodePosition === 1 ? <span className="poemui-timeline__item__date">{node || null}</span> : null}
					</div>

					<div className="poemui-timeline__item__info">
						{title && title !== '' && typeof(title) !== 'undefined' ? <h3>{title}</h3> : null}
						{content && content !== '' && typeof(content) !== 'undefined' ? <div>{content}</div> : null}

						{nodePosition === 0 ? <span className="poemui-timeline__item__date">{node || null}</span> : null}
					</div>

					<div className="poemui-timeline__line"></div>
