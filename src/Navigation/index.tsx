...
export default class Navigation extends Component<NavigationProps, NavigationState> {
    ...
    handleScrollEvent() {
		const $elMenuContainer = __( '.poemui-menu__container:not(.is-mobile)' );
		const $elMobileToggle = __( '.admin-bar .poemui-menu-mobile__toggle' );
	...
	handleClickMobileToggle(e) {
		...
		if ( el.classList.contains( 'is-active' ) ) {
			document.body.classList.add( 'js-poemui-menu-opened' );
		} else {
			document.body.classList.remove( 'js-poemui-menu-opened' );
		}
	}
	...
	createMenu(mobileEnabled) {
		...
		let navClassName = direction === 'vertical' ? 'poemui-v-menu__container' : 'poemui-menu__container';
		if ( mobileEnabled ) navClassName = 'poemui-menu__container';
			...
			<>
				...
				ref={(node) => {
					...
					<div className="poemui-menu__inner">
						...
						{mobileLogo ? <><span className="poemui-brand--mobile"><img src={mobileLogo} alt={label as string} /></span></> : ''}
						<MenuList menuListData={data} mobile={mobileEnabled} dir={direction} />
						{tools ? <><div className="poemui-menu__right-box">{tools}</div></> : ''}
	...
	render() {
		...
		return (
		  <>
			...
			{displayMobileNav ? (
				<>
					<div ref={this.mobileToggleRef} className="poemui-menu-mobile__toggle" onClick={this.handleClickMobileToggle}>
						<span></span>
						<span></span>
						<span></span>
					</div>
					<div ref={this.mobileMaskRef} className="poemui-menu-mobile__mask" onClick={this.handleClickMobileClose}></div>
				</>
			) : ''}
