	...
	render() {
		...
		const radioOptionsList = optionKeys.map((radioOption, index) => {
			...
			if ( index === defaultValIndex ) {
				return <label key={index} className={index === defaultValIndex ? 'is-active' : ''}><input type="radio" name={nameRes} value={optionValues[index] as string} required={requiredVal} defaultChecked /><span className="poemui-controls__radio-trigger"></span>{radioOption}</label>;
			} else {
				return <label key={index} className={index === defaultValIndex ? 'is-active' : ''}><input type="radio" name={nameRes} value={optionValues[index] as string} required={requiredVal} /><span className="poemui-controls__radio-trigger"></span>{radioOption}</label>;
			}
		...
		return (
		  <>
				...
				<div className={"poemui-controls poemui-controls__radio" + wrapperClassDisabled } id={idRes} {...attributes}>
			       {radioOptionsList}
				</div>
