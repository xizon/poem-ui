...
const Toast = (props) => (
    <div
        className={`poemui-toast ${props.removed ? 'is-animating' : ''}`}
        data-index={props.index}
        style={{
            transform: `perspective(100px) translateZ(-${2 * props.index}px) translateY(${35 * props.index}px)`,
            zIndex: props.depth
        }}>
        <div className="poemui-toast__content">
            {props.title === '' || props.title === false ? '' : <><h4 className="poemui-toast__title">{props.title}</h4></>}
            <p className="poemui-toast__message">{props.message}</p>
        </div>
        <svg onClick={() => { props.closeEv(props.index) }} className="poemui-toast__closebtn" width="12px" height="12px" viewBox="0 0 16 16"><path d="M9.41 8l3.29-3.29c.19-.18.3-.43.3-.71a1.003 1.003 0 00-1.71-.71L8 6.59l-3.29-3.3a1.003 1.003 0 00-1.42 1.42L6.59 8 3.3 11.29c-.19.18-.3.43-.3.71a1.003 1.003 0 001.71.71L8 9.41l3.29 3.29c.18.19.43.3.71.3a1.003 1.003 0 00.71-1.71L9.41 8z" fillRule="evenodd"></path></svg>
    </div>
);
        ...
        return (
            <>
                <div id={id || this.uniqueID}
                    className={`poemui-toaster__container poemui-toaster__container--${direction ? direction : 'center'}`}>
                    <div className="poemui-toaster">
                        {toasts.map((item, i) => {
                            return <Toast
                                depth={depth - i}
                                key={i}
